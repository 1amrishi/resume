...
import { RESUME_DATA_YELLOW } from "@/data/resume-data-yellow";
      ...
      <section className="mx-auto w-full max-w-2xl space-y-8 bg-white print:space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex-1 space-y-1.5">
            ...
            <div className="hidden flex-col gap-x-1 font-poppins text-sm text-muted-foreground print:flex">
              {RESUME_DATA_YELLOW.contact.email ? (
                <a href={`mailto:${RESUME_DATA_YELLOW.contact.email}`}>
                  <span className="underline">{RESUME_DATA_YELLOW.contact.email}</span>
                </a>
              ) : null}
              {RESUME_DATA_YELLOW.contact.tel ? (
                <a href={`tel:${RESUME_DATA_YELLOW.contact.tel}`}>
                  <span className="underline">{RESUME_DATA_YELLOW.contact.tel}</span>
                </a>
              ) : null}
            </div>
          </div>
          ...
          <Avatar className="size-28">
            <AvatarImage alt={RESUME_DATA_YELLOW.name} src={RESUME_DATA_YELLOW.avatarUrl} />
        ...
        <Section className="print-force-new-page scroll-mb-16">
          ...
          <div className="-mx-3 grid grid-cols-1 gap-3 print:grid-cols-3 print:gap-2 md:grid-cols-2 lg:grid-cols-3">
            {RESUME_DATA_YELLOW.projects.map((project) => {
              return (
                <ProjectCard
                  key={project.title}
                  title={project.title}
